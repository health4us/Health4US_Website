...
export const PageProvider: ProviderCreator<IPageReflector> = (container) => {
	...
	const shouldCleanPageStructureOnPageDidUnmount = !!experiments[
		'specs.thunderbolt.cleanPageStructureOnPageDidUnmount'
	]
	featureState.update(() => ({}))
	...
	const destroyPage = async (contextId: string) => {
		...
		if (!shouldCleanPageStructureOnPageDidUnmount) {

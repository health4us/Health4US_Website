...
export class ScreenInManager {
	...
	_hideComponent(compId: string) {
		const element = document.querySelector(`#${compId}`) as HTMLElement | null
		if (element) {
			element.style.opacity = '0'
		} else {
			const elements: any = Array.from(document.querySelectorAll(`[id^="${compId}__"]`))
			if (elements?.length) {
	...
	_addAnimatingClass(targetId: string) {
		...
		if (el) {
			...
			const elements: any = Array.from(document.querySelectorAll(`[id^="${targetId}__"]`))
			if (elements?.length) {

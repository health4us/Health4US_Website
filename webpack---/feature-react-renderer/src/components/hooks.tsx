...
const useControllerHook = (displayedId: string, compType: string, _compProps: CompProps, stateValues: CompProps) => {
	const { getCompBoundedUpdateProps, getCompBoundedUpdateStyles, compControllers, experiments } = useContext(Context)
	...
	const shouldYieldToMain =
		!!experiments['specs.thunderbolt.YieldToMainInFunctionPropsHandler'] &&
		!YIELD_TO_MAIN_COMPONENTS_BLACK_LIST.includes(compType)
